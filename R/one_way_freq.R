...
#' @param n.classes numeric. the number of classes for the numeric variable
...
#' @param use.hist.classes logical. uses the default classes for histograms, allowing the frequency table of numeric values to match the default graphical bins for an histogram.
#' @param count.na logical. include NA values as part of the classes to be counted. If the vector has no missing values it will report 0 occurrences when TRUE is selected-
#' @param n.digits integer. number of decimal places to display for percentage frequencies. The number of decimals shown for relative frequencies will be digits + 2.
...
#' @param show.cumulative logical. Show or hide the column for cumulative frequencies, default is TRUE.
#' @param show.rel.cumulative logical. Show or hide the column for relative cumulative frequencies, default is TRUE.
#' @param show.frequencies logical. Show or hide the counts per class, default is TRUE.
...
#' @param x.as.categorical logical. Force the presentation of a continuous variable as a categorical one.
...
one.way.freq <- function(x = vector(),
  ...
  if (isTRUE(compare.valids)) {
    ...
    } else if ((var.class %in% 
                c("factor", "character", "logical", "categorical")) &
               is.null(svy.design)) {
      freqs.na <- table(classes = x,
                        exclude = FALSE,
                        useNA = "ifany")
    } else if (isTRUE(class(svy.design)[1] %in% 
    ...
    if ((var.class %in% c("double", "numeric", "integer"))  &
      ...
      freqs <- table(classes = x,
                     ...
                     useNA = "no")
                      ...
                      c("survey.design2", "survey.design", "svyrep.design"))) {
      freqs <- svytable(as.formula( paste0( "~" , x)),
                        design = svy.design, round = TRUE)
    }
    ...
    if ((var.class %in% c("double", "numeric", "integer"))  &
      ...
      freqs <- table(classes = x,
                     ...
                     useNA = use)
                      ...
                      c("survey.design2", "survey.design", "svyrep.design"))) {
      freqs <- svytable(as.formula( paste0("~", x)),design = svy.design, 
                        round = TRUE)
    }
  }
  ...
  not.categorical <-
    !(var.class %in% c("factor", "character", "logical"))
  continuous <-
    ((
      var.class %in% c("double", "numeric",
                       "integer")
    ) & 
      (length(unique(x)) > 15))
  if (not.categorical) {
