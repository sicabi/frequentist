...
frequencies <- function(x = vector(),
  ...
  table.type <- ifelse(    isTRUE(show.frequencies) &
    ...
    if (isTRUE(use.hist.classes)) {
      ...
      if (n.classes >= 1) {
        if (n.classes != nclass.Sturges(x)) {
          n.breaks <- seq(from = round(min(x, na.rm = TRUE),0),
  ...
  if (isTRUE(compare.valids)) {
    counts.table <- function(freqs, freqs.na, type) {
      switch(type,
             ...
             only.rel.cumulative = cbind(rel.cumulative.freqs.n =
                                           cumsum(prop.table(freqs.na)),
                                         rel.cumulative.freqs.valid =
                                           cumsum(prop.table(freqs))),
             only.cumulative = cbind(cumulative.freqs.n = cumsum(freqs.na),
                                     cumulative.freqs.valid = cumsum(freqs)),
             only.cumulatives = cbind(cumulative.freqs.n = cumsum(freqs.na),
                                      cumulative.freqs.valid = cumsum(freqs),
                                      rel.cumulative.freqs.n =
                                        cumsum(prop.table(freqs.na)),
                                      rel.cumulative.freqs.valid =
                                        cumsum(prop.table(freqs))),
             only.rel.freq = cbind(relative.freqs.n = prop.table(freqs.na),
                                   relative.freqs.valid = prop.table(freqs)),
             only.relatives = cbind(relative.freqs.n = prop.table(freqs.na),
                                    relative.freqs.valid = prop.table(freqs),
                                    rel.cumulative.freqs.n =
                                      cumsum(prop.table(freqs.na)),
                                    rel.cumulative.freqs.valid =
                                      cumsum(prop.table(freqs))),
             relative.and.cumulative = cbind(relative.freqs.n =
                                               prop.table(freqs.na),
             ...
             cumulative.freqs.valid =
               cumsum(freqs),
             relative.and.cumulatives = cbind(relative.freqs.n =
                                                prop.table(freqs.na),
                                              relative.freqs.valid =
                                                prop.table(freqs),
                                              cumulative.freqs.n =
             ...
             only.freqs = cbind(freqs.n = freqs.na,
                                freqs.valid = c(freqs)),
             freqs.and.rel.cumulative = cbind(freqs.n = freqs.na,
                                              freqs.valid = freqs,
                                              rel.cumulative.freqs.n =
                                                cumsum(prop.table(freqs.na)),
                                              rel.cumulative.freqs.valid =
                                                cumsum(prop.table(freqs))),
             freqs.and.cumulative =  cbind(freqs.n = freqs.na,
                                           freqs.valid = freqs,
                                           cumulative.freqs.n =
                                             cumsum(freqs.na),
                                           cumulative.freqs.valid =
                                             cumsum(freqs)),
             freqs.and.cumulatives = cbind(freqs.n = freqs.na,
                                           freqs.valid = freqs,
                                           cumulative.freqs.n =
                                             cumsum(freqs.na),
                                           cumulative.freqs.valid =
                                           ...
                                           rel.cumulative.freqs.valid =
                                             cumsum(prop.table(freqs))),
             freqs.and.relative = cbind(freqs.n = freqs.na,
                                        freqs.valid = freqs,
                                        relative.freqs.n =
                                          prop.table(freqs.na),
                                        relative.freqs.valid =
                                          c(prop.table(freqs))),
             freqs.and.relatives = cbind(freqs.n = freqs.na,
                                         freqs.valid = freqs,
                                         relative.freqs.n =
                                           prop.table(freqs.na),
                                         relative.freqs.valid =
                                         ...
                                         rel.cumulative.freqs.valid =
                                           cumsum(prop.table(freqs))),
             freqs.relative.and.cumulative = cbind(freqs.n = freqs.na,
                                                   freqs.valid = freqs,
                                                   relative.freqs.n =
                                                     prop.table(freqs.na),
                                                   relative.freqs.valid =
    ...
    counts.table <- function(freqs, type) {
      switch(type,
             ...
             only.cumulatives = cbind(cumulative.freqs = cumsum(freqs),
                                      rel.cumulative.freqs =
                                        cumsum(prop.table(freqs))),
             only.rel.freq = cbind(relative.freqs = prop.table(freqs)),
             only.relatives = cbind(relative.freqs = prop.table(freqs),
                                    rel.cumulative.freqs =
                                      cumsum(prop.table(freqs))),
             relative.and.cumulative = cbind(relative.freqs =
                                               prop.table(freqs),
                                             cumulative.freqs =
                                               cumsum(freqs)),
             relative.and.cumulatives = cbind(relative.freqs =
                                                prop.table(freqs),
                                              cumulative.freqs =
                                                cumsum(freqs),
                                              rel.cumulative.freqs =
             ...
             freqs.and.cumulative =  cbind(freqs,
                                           cumulative.freqs =
                                             cumsum(freqs)),
             freqs.and.cumulatives = cbind(freqs,
                                           cumulative.freqs =
                                             cumsum(freqs),
                                           rel.cumulative.freqs =
                                             cumsum(prop.table(freqs))),
             freqs.and.relative = cbind(freqs,
                                        relative.freqs =
                                          prop.table(freqs)),
             freqs.and.relatives = cbind(freqs,
                                         relative.freqs =
                                           prop.table(freqs),
                                         rel.cumulative.freqs =
                                           cumsum(prop.table(freqs))),
             freqs.relative.and.cumulative = cbind(freqs,
                                                   relative.freqs =
                                                     prop.table(freqs),
                                                   cumulative.freqs =
                                                     cumsum((freqs))),
  ...
  if (table.type != "none") {
    freq.table[,
               grep(pattern = "rel", x = colnames(freq.table),
                    value = FALSE)] <-
      round(freq.table[,
                       grep(pattern = "rel",
  ...
  if (isFALSE(compare.valids)) {
    if (table.type != "none") {
      freq.table[nrow(freq.table),
                 grep(pattern = "cumul", x = colnames(freq.table),
                      value = FALSE)] <- c(NA)
    }
  } else {
    if (table.type != "none") {
